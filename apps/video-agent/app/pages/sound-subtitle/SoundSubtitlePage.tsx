...
function VoiceCatalogSelect({
  ...
  function toggleLanguageFilter(value: VoiceLanguageFilter) {
    setLanguageFilter((current) => current === value ? null : value);
    setActiveIndex(0);
  }

  function toggleGenderFilter(value: VoiceGenderFilter) {
  ...
  return (
    <div className="voiceCatalogSelect" ref={rootRef}>
      <button
        ref={triggerRef}
        type="button"
        role="combobox"
      ...
      {open && (
        <div className="voiceCatalogPopover">
          <div className="voiceCatalogPopoverHeader">
            <strong>选择音色</strong>
            <span>{filteredEntries.length === voices.length ? `${voices.length} 个可用` : `${filteredEntries.length} / ${voices.length} 个可用`}</span>
          </div>
          <input
            ref={searchRef}
            type="search"
              ...
              <div className="voiceCatalogFilterTags">
                {voiceLanguageFilters.map((filter) => (
                  <button
                    ...
                    title={filter.title}
                    key={filter.value}
                    onClick={() => toggleLanguageFilter(filter.value)}
                  >
                    {filter.label}
                  </button>
                ))}
              ...
              <div className="voiceCatalogFilterTags">
                {voiceGenderFilters.map((filter) => (
                  <button
                    ...
                    key={filter.value}
                    onClick={() => toggleGenderFilter(filter.value)}
                  >
                    {filter.label}
                  </button>
                ))}
...
function extractLanguageOptions(value: unknown): CatalogOption[] {
  if (!Array.isArray(value)) return [];
  const options = value.flatMap((item): CatalogOption[] => {
