...
- 2026-07-14 用户确认 TTS 生成前需要选择声音配置，页面上方应根据当前语音模型的真实能力目录展示音色、语言/口音和可调参数；情绪风格只有在目录存在非空有效值且当前协议有官方请求字段映射时才允许展示，不得由 LLM 猜测或前端硬编码。模型切换后动态刷新且不得静默替换失效选择，Agent 可推荐但用户必须查看、试听并确认；需要实时调用模型的试听必须由用户主动触发。
...
- 2026-07-16 用户纠正并确认已有/上传音频调用 `volc.seedasr.auc` 时采用系统公用的私有 TOS 临时中转：TOS 归属 Admin“工具与 MCP”控制面，独立于全部 ASR 模型，系统维护一个启用的版本化配置供所有 ASR 模型共用；ASR 模型新增、编辑、停用和删除不得携带或受 TOS 配置约束。每个任务幂等上传，向 ASR 只提交短期签名 GET URL，任务终态后删除临时对象，清理失败必须保留审计并可重试；存在待清理对象时只阻止 TOS 配置变更或停用，不阻止模型 CRUD。TOS 仅是外部处理暂存，不替代素材库的自管存储，也不得把 TOS 凭据、签名 URL 或查询参数写入模型快照、素材 metadata、Agent 消息或日志。真实 Bucket 连接检查由独立且默认关闭的 `TOS_TOOL_WORKER_ENABLED` 执行，不得为检查连接而开启可能计费的语音任务消费。
...
- 2026-07-16 `add-sound-subtitle-generation` 当前为 `33/35`：TTS/ASR、目录同步、声音/字幕工作台及系统公用私有 TOS 已完成；视频工作台已使用数据库菜单 `route_path` 持久化一级/二级路由，支持深层直达、刷新和浏览器前进/后退恢复。音色目录同步已从语音生成 Worker 解耦，当前环境 `VOICE_CATALOG_WORKER_ENABLED=true`，真实 `ListSpeakers` 同步成功获取 14 页、415 个音色；工作台音色选择使用可搜索单选组合框，展示供应商原名、中文描述以及中文性别/年龄/语言标签，供应商未提供中文名时不得伪造；语言只按 `Languages[].Language` 的真实代码映射中文并提交原代码，禁止把 `Languages[].Text` 试听文案当成语言名，并使用位于触发框下方的自定义单选弹层。音色弹层按 `中文/英文/多语言` 与 `男声/女声` 两排可取消 Tag 做交集筛选，列表保持扁平；`多语言` 包含其他单语种、语言未知及真正多语言音色，保证真实目录不被隐藏。当前 `volcengine_tts_v3` 只向上游发送 `explicit_language`，不得发送未定义的 `language/emotion`；空情绪占位不展示、不强制，非空结构化情绪请求由 Backend 返回 `emotion_unsupported`，数据库 nullable 字段仅保留历史审计。声音与字幕桌面页 1440px 保持 `1118px` 与 `250/520/276px`，1920px 使用 `1598px` 与 `250/1000/276px`，不再居中旧宽度留下大块空白；宽屏中栏按已确认原型使用 `484/462px` 顶部同高、`650/302px` 音色语言、`964×180px` 旁白、`154/200/586px` 底部控制行及 `964×70px` 当前任务；两基准之间在中栏内容宽达到 `700px` 后连续使用双区布局，试听与模型目录同排、操作与语速同排并占满剩余宽度，不得因未达到 1920 临界值退回半宽单列。右栏 Agent 头部固定为“标题和在线状态 / 会话信息和模型选择”两行且不得重叠；目录未声明 `speed_ratio.default` 时初始语速使用合法范围内的 `1.0`，不得用区间中点推导。任务列表只在左栏，中栏包含目录、试听和当前任务状态，右栏包含 Agent 会话与运行审计，底部重复任务表已删除。供应商可选数组 `null` 会归一化为 `[]`，失败自动调度按配置间隔退避，未知异常摘要仅保留异常类型。TOS 已从 `ai_models`、模型 DTO/表单和 Worker 模型配置彻底移除，ASR 任务锁定独立配置版本，Admin“工具与 MCP”负责版本化配置与真实连接检查；系统 TOS 已完成一次受控真实能力检查并成功清理探针。仅剩经单独许可的受控真实 TTS/ASR 验证及其后归档申请；`SPEECH_GENERATION_WORKER_ENABLED=false`、`TOS_TOOL_WORKER_ENABLED=false`。
- 2026-07-16 用户确认声音与字幕工作台从“脚本创作”导入已有旁白：只展示当前项目 `draft | approved` 脚本，单选脚本并默认全选非空分镜，按 `sequence` 合并后明确替换当前旁白；超出当前 TTS 模型字符上限时阻止导入且不得截断。试听/生成预检提交脚本 ID、详情 `updated_at` 和所选分镜 ID，服务端必须重新读取并校验项目、状态、版本和分镜归属，任务分别保存可编辑最终文本与不可变原始旁白来源快照；脚本变化或归档时阻止预检，失败任务重试复制历史来源快照而不得读取最新脚本覆盖。
- 2026-07-17 用户确认当前本地环境启用语音任务自动执行：`SPEECH_GENERATION_WORKER_ENABLED=true`；继续保持 `VOICE_CATALOG_WORKER_ENABLED=true`、`TOS_TOOL_WORKER_ENABLED=false`。启用后声音任务可能产生真实供应商调用，永久鉴权错误不得自动重试。
- 2026-07-17 用户纠正声音工作台 TTS 模型原生选项层与触发框样式错位；TTS 模型选择改用工作台自定义单选弹层，必须在触发框正下方同宽展开，并支持外部点击、`Escape` 和键盘选择，不得回退为浏览器原生 `select` 选项层。
- 2026-07-17 用户确认 TTS 中转模型可在 `api_protocol`、`upstream_model` 和 `resource_id` 均与官方目录模型一致时复用同一音色目录（例如 `doubao-seed-tts-2.0 / seed-tts-2.0`），中转模型不要求 OpenAPI AK/SK。实现采用显式官方根目录来源关系，禁止自引用、共享链和不匹配绑定；目录读取、同步、声音 Agent、TTS 预检及任务快照统一解析并审计实际来源，仍被共享时阻止来源停用、删除或改变目录身份。
- 2026-07-17 用户确认 ZeekAI/New API 类 TTS 中转采用独立 `openai_audio_speech` 协议：使用 Bearer API Key 与 `/v1/audio/speech`，必须按相同 `upstream_model + resource_id` 复用官方 `volcengine_tts_v3` 根目录，不得冒充原生 V3。该协议首版只生成音频，`supports_word_timestamps=false`；TTS 同步字幕必须明确阻断，不伪造时间轴，也不自动追加 ASR，操作者仍可显式使用已有音频 ASR。
- 2026-07-17 用户确认声音与字幕桌面工作台采用 Pencil `wl2QV` 可读字号层级：常规正文不低于 `12px`、辅助信息不低于 `11px`，任务标题、表单值、主要按钮和失败标题为 `13px`，旁白正文为 `14px`，失败供应商消息/诊断字段为 `12/11px`；保持已确认三栏宽度、宽屏中栏结构、`140px` 宽屏失败详情和左栏独立滚动，不得通过缩小字号维持密度。
- 2026-07-17 `add-sound-subtitle-generation` 经用户明确确认以 `45/47` 强制归档到 `openspec/changes/archive/2026-07-17-add-sound-subtitle-generation/`，主规格已同步。用户明确放弃任务 4.4 的最小受控真实 TTS/ASR 验证和任务 4.5 的最终全量验收；归档不得解释为真实供应商链路已成功。归档前真实 TTS 尝试分别出现中转 HTTP 403 与另一中转域名 TLS `unexpected eof while reading`，ASR 未完成真实验证；后续若恢复真实验收必须新建独立 OpenSpec change，并重新取得成本许可，不得修改归档任务为已完成。
