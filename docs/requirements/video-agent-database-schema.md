---
name: video-agent-database-schema
...
- 已新增 SQLx 初始迁移：`backend/migrations/20260701000000_initial_schema.sql`
- 已在运行中的 `video_agent` PostgreSQL 数据库执行迁移，`_sqlx_migrations` 记录版本为 `20260701000000 initial schema`
- 当前初始 schema 覆盖 `projects/accounts/materials/material_embeddings/scripts/scenes/generation_tasks/videos/publish_tasks/metrics/revenues/agent_runs/agent_steps/viral_videos/content_strategies`
- 迁移验证测试位于 `backend/tests/database_migrations.rs`，会创建临时 PostgreSQL 数据库验证核心表、索引和约束
...
- 新增业务先看能否复用现有表
- JSONB字段先用，稳定后再拆成独立表
- 与 [`project-tech-stack`](../memory/project-tech-stack.md) 配合使用
