...
POC 的原则是：基建骨架完整，业务功能切片收窄。也就是说，M0 必须把租户、权限、模型、Run、Trace、Job、Provider、Tool Policy、Eval Dataset 这些基础边界搭好；M1/M2 只选择员工培训系统这一个场景把闭环跑通。
...
目标：完成 Codex-like 的工具使用循环，并跑通底层 Run Graph 状态机。
