...
系统 SHALL 通过 `AgentExecutionContext` 向 Adapter 提供本轮 conversation、project、subject、run、模型执行引用和 Step Recorder，且 SHALL NOT 暴露其他 Agent 的业务 Repository。
...
#### Scenario: Adapter 使用本轮模型快照
- **GIVEN** Coordinator 已为本轮解析启用的文本模型
- **WHEN** Adapter 执行一个或多个模型步骤
- **THEN** 所有步骤 SHALL 使用本轮相同的模型执行引用
- **AND** 系统 SHALL 保持现有禁止自动跨模型切换的规则
