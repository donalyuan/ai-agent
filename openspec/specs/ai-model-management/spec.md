...
模型记录 SHALL 保存 `api_protocol`、`protocol_version`、`auth_scheme` 和 `request_base_url`，运行时 SHALL 仅根据 `api_protocol` 选择 adapter。
...
- **WHEN** 操作者为图片模型选择 `openai_responses` 或为文本模型选择 `jimeng_visual`
- **THEN** 系统 SHALL 拒绝保存
...
- **WHEN** 系统解析一个可调用模型
- **THEN** 系统 SHALL 根据 `api_protocol` 选择请求结构和响应解析器
...
- **GIVEN** 操作者读取了模型当前 `version`
- **WHEN** 操作者提交合法字段和匹配版本
- **THEN** 系统 SHALL 更新模型
- **AND** 系统 SHALL 递增 `version`

#### Scenario: 并发编辑冲突

- **GIVEN** 模型已被另一请求更新
- **WHEN** 操作者使用旧 `version` 再次提交编辑
- **THEN** 系统 SHALL 拒绝覆盖
- **AND** 系统 SHALL 返回 `model_version_conflict`

#### Scenario: 按类型和状态筛选

- **WHEN** 操作者选择模型类型、状态、供应商、协议或输入关键词
- **THEN** 管理列表 SHALL 只返回匹配记录
- **AND** 列表 SHALL 展示默认、启停、最近调用和更新时间
