...
- [x] 8.1 使用 TanStack Table + shared/ui DataTable/filters/tabs 展示目录、版本和 usage；覆盖稳定 cursor、六类过滤、授权/processing diagnostic 与 keyboard/ARIA。
- [x] 8.2 使用 TanStack Virtual 渲染长目录、版本历史和 usage，证明 DOM 有界、滚动/选择不丢 project scope，且不可用 owner 显示 unavailable/partial 而非伪空。
- [x] 8.3 保持音频播放/波形为领域功能，复用 shared/ui 外壳但不将播放器进入 shared/ui；验证 play/pause/seek、grant expiry、Timeline handoff 和零隐式外部调用。
- [x] 8.4 运行资产中心 focused E2E、2 GiB admission/resume 证据、本 change strict validation 与 `git diff --check`；全部任务保持未勾选直至验收。
