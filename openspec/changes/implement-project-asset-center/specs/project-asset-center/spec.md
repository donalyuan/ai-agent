...
- **DDD**：目录、上传、派生和 usage 分别引用权威 owner facts。
- **BDD**：目录、上传恢复/取消、试听、版本、usage 和失败状态均可直接验收。
- **SDD**：固定路由、分页/filter、CAS、operation/reservation、grant 和 owner isolation。
- **TDD**：domain/contract/HTTP/component/E2E 先覆盖重复、foreign、stale、partial 和零副作用。
