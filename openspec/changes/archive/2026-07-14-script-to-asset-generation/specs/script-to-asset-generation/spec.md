...
定义视频工作台从脚本分镜生成素材候选、选择分镜主素材、控制 AI 图片生成成本和 AI 视频二次确认的第一版能力。
...
系统 SHALL 将脚本生成与素材生成拆分为不同二级菜单入口，素材候选生成不得嵌在脚本生成页内。

#### Scenario: 素材生成独立二级菜单

- **GIVEN** 操作者打开视频工作台菜单
- **WHEN** 操作者展开素材管理
- **THEN** 系统 SHALL 显示 `素材库` 和 `素材生成` 两个独立二级入口
- **AND** `素材生成` SHALL 承载脚本分镜素材候选生成、选择和 AI 视频二次确认
...
- **GIVEN** 操作者查看脚本详情或素材生成页
- **WHEN** 页面展示来源选题、脚本 Agent 对话或素材候选区域
...
系统 SHALL 只为 AI 视频生成创建待确认任务，未确认前不得执行外部视频生成调用。

#### Scenario: 视频任务默认待确认

- **GIVEN** 某分镜需要 AI 视频素材
- **WHEN** 操作者生成素材候选
- **THEN** 系统 SHALL 创建 `draft` 状态的视频生成任务
- **AND** worker SHALL NOT 执行该任务

#### Scenario: 确认后才允许执行视频任务

- **GIVEN** 某 AI 视频任务处于 `draft`
- **WHEN** 操作者二次确认该任务
- **THEN** 系统 SHALL 将任务转为可执行状态
- **AND** 后续 worker 才可执行外部视频生成调用
