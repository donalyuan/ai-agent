...
时间线页面使用共享 `shared/ui` 基础控件、语义 tokens 和 Lucide 工具图标；`react-resizable-panels` 只允许预定义最小/最大分区，dnd-kit 只处理同一父 scope 的 Clip 排序，TanStack Virtual 只处理长列表和日志。PixiJS、WaveSurfer.js、HLS.js 是领域 adapter，分别承载画面、波形和同源代理视频，不能从 `shared/ui` 导出或被其他页面当作通用控件。

验证顺序为面板尺寸/键盘排序/虚拟 DOM 测试、Pixi 非空 Canvas、WaveSurfer 播放位置、HLS loading/ready/error 和 React Flow graph mutation zero-write；随后执行桌面 Timeline/Exports focused E2E 与 strict validation。失败必须保留原始 renderer/media/owner diagnostic，不以 mock 或 fallback 伪造成功。
