...
该 change 先建立 `apps/web/src/shared/ui` 的源码型 shadcn/Radix 基线、Tailwind/CSS Variables 语义 tokens 和 Lucide 图标约束。共享组件提供稳定的基础变体、Dialog/Tabs/Tooltip/Command/Toaster、有限可调面板、DataTable/Form 和 VirtualList；业务 change 只消费这些封装，不在页面内复制变体或新增页面级 CSS。`react-resizable-panels` 只提供预定义分区和有限尺寸，禁止自由停靠 IDE。

固定 published WorkflowVersion 的 300-node 投影由本 change 的领域页面负责：React Flow 仅承载只读节点/边/状态投影，长列表/日志使用 TanStack Virtual，节点拖动、连线、删除、保存、发布和版本升级明确为 MVP-B。React Flow、PixiJS、WaveSurfer.js、HLS.js 保持在各自领域模块，不能从 `shared/ui` 导出。

验证顺序为 shared/ui 可访问性和 token 测试、300-node 投影加载/筛选/详情/导航测试、graph mutation zero-write 测试，再运行桌面 Chrome 与 Edge 和全量 OpenSpec strict validation。Review、Timeline、Provider/Model/Skill 和 Assets 的验证必须引用同一 shared/ui 导出，不允许第二实现。
