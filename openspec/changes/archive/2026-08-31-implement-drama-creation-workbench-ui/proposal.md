...
本 change 是阶段一前端 `shared/ui` 基线的唯一建立者。实现 SHALL 以 shadcn/Radix 源码封装为基础，使用 Tailwind utilities、CSS Variables 语义 tokens 和 Lucide 图标；基础变体、`Dialog`、`Tabs`、`Tooltip`、`Command`、`Toaster`/通知、表格/表单/虚拟列表等共享能力必须从该目录消费。业务页面不得再引入第二套组件库、页面级手写 CSS 或复制基础变体。

本 change 还负责固定 300 个 published Workflow node 的只读 React Flow 投影及其查询/筛选/详情入口。节点移动、连线、删除、保存、发布和版本升级均关闭，通用图编辑继续属于 MVP-B；React Flow、PixiJS、WaveSurfer.js、HLS.js 等领域组件不得进入 `shared/ui`。`react-resizable-panels` 只用于有限分区，不能实现 IDE 自由停靠。

验收必须证明 shared/ui 组件可被 Review、Timeline、Provider/Model/Skill、Assets 页面复用，300-node 投影保持只读且导航零业务写入；同时覆盖键盘/ARIA、稳定尺寸、语义 token、无第二组件库、无 MVP-A graph mutation 和 `openspec validate --changes --strict --json`。
