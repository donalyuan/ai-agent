...
- **DDD**：Session 管理审查生命周期；Candidate/Decision 是追加审计事实；资产版本归既有 Asset 聚合。
- **BDD**：以上场景定义成功接受及所有主要拒绝/冲突结果。
- **SDD**：Schema、API、表、端口、兼容性、总体追溯与失败代码均为该 change 的规范边界。
- **TDD**：每个场景必须先由 domain/application 测试表达，再扩展至 adapter、HTTP、migration 与 BDD；前置缺失和范围漂移也必须有定向失败测试。
...
- **Current**：能力不存在，原版本只追加且可复用。
- **Defined**：需求定义新增功能和不变的兼容边界。
- **Todo**：实现所有规范场景，直到任务清单完成。
