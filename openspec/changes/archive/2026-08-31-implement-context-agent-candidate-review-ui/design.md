...
Review 复用 `shared/ui` 的选择/确认 Dialog、Tabs、Tooltip、Command、Toaster 及已有语义 tokens；不新增基础 Button/Input/Dialog 变体，不建立独立主题或 CSS。Command 只导航到 owner-validated project/Episode scope，Toaster 只反馈已返回的 owner 结果，不能代替持久状态。

组件与 E2E 验证必须覆盖键盘确认、焦点回收、候选 tab 切换、tooltip 诊断、命令 scope、成功/失败通知及 selection 清除，并证明这些交互只更新本 change 的 presentation state 或显式 owner command。
