...
- **DDD**：SoundCue/字幕/ExportJob 归 Episode Timeline；素材和输出均为不可变引用。
- **BDD**：场景覆盖四类导入音频、手工字幕、预检、MP4/SRT/light 及主要失败。
- **SDD**：定义 API、Schema、DB、FFmpeg port、Outbox、兼容性与显式非目标。
- **TDD**：Mock renderer 和 manifest 正反 fixture 先覆盖全部 required 字段、空值/unknown cost source 与 profile alias，真实 FFmpeg 仅在授权 probe 测试。

## Current / Defined / Todo

- **Current**：没有 audio/export application、FFmpeg 或 media worker 业务链路。
- **Defined**：MVP-A light 交付边界。
- **Todo**：实现任务、预检、安全执行和测试。
