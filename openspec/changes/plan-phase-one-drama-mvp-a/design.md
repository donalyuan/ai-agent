...
- 为一个总体协调 change 与十九个 child 建立可追踪的交付顺序，允许无共享写入的切片并行；总体协调不是 child 实施运行时必须先应用的代码依赖。
...
### 1. 以一个总体协调 change 与十九个 child 管理阶段 1

`plan-phase-one-drama-mvp-a` 只拥有总体 proposal/design/spec/tasks、矩阵、DAG、验收和待确认项。它提供协调与可追溯约束，不是 child 实施运行时必须先应用的代码依赖，也不要求 child 在其 tasks 完成或归档后才能实施。十九个 child 各自拥有其领域模型、接口、迁移、UI 或测试，不能将其他 child 的实体或运行时副作用纳入自身实现。
...
| `fix-phase-one-review-vulnerabilities` | 7.12 | 阶段一 review 发现的 scope、CAS、运行链、Storage binding 与 no-GC 完整性修复 | 阶段一各 owner 已发布合同 | 后置各相关 owner；只修复已证实缺口，不创建新产品能力 |
...
plan（协调/追溯，非实施运行时代码依赖）
 ...
 └─ confirmed UI architecture ── drama creation UI shared/ui baseline
...
1. 十九个 child 可按 DAG 独立创建和实施；本总体 change 的 artifacts 提供协调与追溯，不要求先应用、完成或归档其 tasks。并行 change 不共享目标目录和 Alembic revision。共享 UI 基线由 `implement-drama-creation-workbench-ui` 建立，其他正式 UI change 只消费该基线，不增加独立原型路由或第二套组件库。
...
- **DDD**：总体 plan 仅协调十九个 child 的领域所有权与依赖，不成为运行时模块；projects 持有 Project/creationMode/CreativeBrief/项目设置/预算与 Story/Script current refs，AssetBible 独立持有 entry/version/override/snapshot/impact/task，text 只读消费 validated brief snapshot并持有 adaptation SourceMaterial/candidate/TextReview；AssetEdit 只拥有 image/video，TextReview 通过 accepted handoff + aggregate owner typed command/ack 落地，Timeline editor 保持独立 owner；Asset metadata/reservation、Storage session/object、Media derivative 与 usage query projection 分离；settings UI、operations resilience 与 local observability 的 owner 边界固定，telemetry 不拥有业务状态；共享 UI 基线由创作工作台建立，其他正式 UI change 不拥有共享基础组件或业务领域事实。
...
- **Current**：阶段 0 已实现。阶段 1 当前为 1 overall + 19 child = 20 个未归档 changes，475 项 task 均已完成；共享 UI 基线已归入正式创作工作台 change。本次不回退任何既有实现、主规格或证据。
- **Defined**：本总体 plan 的冻结契约、DAG、退出条件和 probe 输入。
- **Todo**：按 DAG 实施 19 个 child，并完成集成验证。
